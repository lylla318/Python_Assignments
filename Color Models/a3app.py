...
def round5_cmyk(cmyk):
    """Proxy function for a3.round5_cmyk"""
    result = a3.round5_cmyk(cmyk)
    if result is None:
        return ''
    return result 

def round5_hsv(hsv):
    """Proxy function for a3.round5_hsv"""
    result = a3.round5_hsv(hsv)
    if result is None:
        return ''
    return result 
...
class TopPanel(BoxLayout):
    ...
    def update(self, rgb, cmyk, hsv):
        ...
        cmyk_str = '' if cmyk is None else round5_cmyk(cmyk) 
        hsv_str  = '' if hsv is None else round5_hsv(hsv)
